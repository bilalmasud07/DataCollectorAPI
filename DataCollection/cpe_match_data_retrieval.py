...
import config.cpe_match_conf as cpe_match_info
from utils.data_fetcher import fetch_data
from utils.data_processor import process_cpe_match_info
...
def main():
    ...
    logfile_name = f"data_retrieval_cpe_match_{formatted_current_date}.log"
    ...
    api_params = cpe_match_info.Params['CPE_MATCH_INFO']
    api_url = api_params['URL']
    csv_files = cpe_match_info.Params['STORE_DATA']
    headers = cpe_match_info.Params['HEADERS']
    data_folder = cpe_match_info.Params['SUB_DIR']['dir_name']

    delete_file(csv_files, data_folder)

    api_key = os.getenv('CVE_API_KEY')

    extract_data(api_url, api_key, csv_files, process_cpe_match_info, headers, data_folder)
...
def extract_data(api_url, api_key, csv_files, process_function, headers, data_folder):
    ...
    while start_index <= total_results:
        ...
        start_index += 10000 # maximum data to be fetched, defined by the API
